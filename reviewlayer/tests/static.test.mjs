...
test('manual email notifications keep addresses server-side and require browser identity', async () => {
  ...
  assert.match(service, /listPinStatusEvents/);
  assert.match(service, /'solved'/);
  assert.match(service, /'reopened'/);
  assert.match(mailer, /function_exists\('mail'\)/);
  assert.match(mailer, /groupNotificationEvents/);
  ...
  assert.match(jsonStorage, /'status_events'/);

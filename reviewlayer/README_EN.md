...
ReviewLayer 1.4.1 is an independent annotation overlay for website prototypes. Clients can attach a note to a page element, discuss it, resolve it, and revisit it on another device. Installation needs no Node.js, build process, MySQL, CDN, or SaaS service.
...
A browser that has not yet established a commenter in the project also shows an optional email field with its first pin or first reply. Supplying it saves the contribution first and then sends one verification link; leaving it empty skips the step. If email delivery fails, the pin or reply remains saved and verification can be retried in Settings. Confirming the same email on another device links the browser to the existing commenter.
